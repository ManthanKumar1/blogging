...
let Home = () => {
    ...
    useEffect(() => {
        let getPost = async () => {
            try {
                setLoading(true)
                let response = await axios.get(`/postList?page=${currentPage}&search=${searchValue}`)
    ...
    let handleSearch = async (e) => {
        ...
        let response = await axios.get(`/postList?q=${input}&page=${currentPage}`)
        ...
        <div>
            ...
            <div className='flexbox-container wrap'>
                {loading ? "Loading..." : posts.map((post) => (
                    <div className='post-card' key={post._id} onClick={() => navigate(`/posts/detail-post?id=${post._id}`)}>
